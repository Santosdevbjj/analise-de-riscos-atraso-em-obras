...
def get_modern_engine(url):
    try:
        ...
        logger.info("Conectando ao banco com Psycopg 3 (Modo Compatibilidade Render)...")
        
        return create_engine(
            new_url,
            pool_pre_ping=True,
            echo=False,
            # 'connect_args' abaixo força o uso de IPv4 e desabilita GSSAPI que causa lentidão
            connect_args={
...
async def analise_preditiva(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if result:
            ...
            pdf.seek(0)
            await update.message.reply_document(document=InputFile(pdf, filename=f"Rel_Gerencial.pdf"))
        else:
            await update.message.reply_text("❌ Obra não encontrada.")
    except Exception as e:
        logger.error(f"Erro: {e}")
        await update.message.reply_text("⚠️ Erro ao acessar banco de dados.")
...
async def run_telegram_bot():
    try:
        # Tenta criar tabelas apenas na inicialização do bot para não travar o app principal
        try:
            Base.metadata.create_all(bind=engine)
        except:
            logger.warning("Não foi possível criar tabelas agora, o banco pode estar offline.")
...
async def health_check():
    return {"status": "online", "driver": "psycopg3_ipv4_forced"}
